...
[![FAIR checklist badge](https://fairsoftwarechecklist.net/badge.svg)](https://fairsoftwarechecklist.net/v0.2?f=21&a=30112&i=31300&r=123)
...
Additionally, for experiments with synthetic fMRI, MindEye models and data must be downloaded from [HuggingFace](https://huggingface.co/datasets/pscotti/mindeyev2), specifically, files `betas_all_subj0X_fp32_renorm.hdf5` (where X is the subject index), `file`and `file` and stored in directory XXXX. For optional result visualization using neuroimaging libraries, we used the file `brain_region_masks.hdf5`.
