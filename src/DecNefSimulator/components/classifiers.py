...
# -*- coding: utf-8 -*-
"""
Implementing Supervised classifiers used in 
...
class BinaryDataLoader(DataLoader):
    def __init__(self, dataset, combo=None, batch_size: int = 1, shuffle: bool = None,
        ...
        if isinstance(combo, list):
            mask = np.logical_or(self.dataset.targets.numpy(
            ) == combo[0], self.dataset.targets.numpy() == combo[1])
        ...
        self.classes = np.array(combo) if isinstance(
            combo, list) else self.dataset.targets.unique()
...
class CNNClassification(ImageClassificationBase):
    def __init__(self, image_shape, classes, device='cuda', name='CNN classifer'):
        ...
        n_channels, resolution_x, resolution_y = self.image_shape
        ...
        with torch.no_grad():
            dummy_input = torch.zeros(
                1, n_channels, resolution_x, resolution_y)
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(1024, 512),
            nn.ReLU(),
            # Since F.cross_entropy() expects raw logits, there’s no need for softmax.
...
class ElasticNetLinearClassification(torch.nn.Module):
    ...
    def l1_reg(self):
        l1_norm = torch.stack([p.abs().sum()
                              for p in self.network.parameters()]).sum()
        return self.l1_lambda * l1_norm

    def l2_reg(self):
        l2_norm = torch.stack([p.pow(2).sum()
                              for p in self.network.parameters()]).sum()
    ...
    def training_step(self, batch):
        ...
        y_hat = self(x)
        if len(self.classes) > 2:
            y = F.one_hot(y.long(), num_classes = len(self.classes)).float()
